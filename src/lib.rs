    ...
    >
{
    ...
    for i in 0..inode.blocks_used() {
        ...
        for byte in 0..self.bytes_per_block() {
            self.file_content_buffer[i*self.bytes_per_block()+byte] = self.block_buffer[byte];
        }
    }
    ...
    for i in 0..inode.blocks_used() {
        ...
        for j in 0..BLOCK_SIZE {
                self.block_buffer[j] = self.file_content_buffer[i * self.bytes_per_block() + j];
        }
        self.disk.write(inode.blocks[i] as usize, &self.block_buffer).unwrap();
    }
    }
    ...
    let mut inode = Inode::<MAX_FILE_BLOCKS, BLOCK_SIZE> {
        bytes_stored: num_entries as u16, 
    ...
    for i in 0..MAX_FILE_BLOCKS {
        ...
        inode.blocks[i] = block_index as u8;
        block_index += 1; 
    }
    ...
    fn initialize_new_file(
        ...
        let new_inode = Inode::new(first_block); 
    ...
    {
        let total_files = self.find_lowest_zero_bit_in(INODE_FULL_BLOCK)
    ...
    let files_to_list = total_files - 1;
    for i in 0..files_to_list {
        let file_index = i + 1; // skip 0th entry (directory itself)
        for byte_index in 0..MAX_FILENAME_BYTES {
            let content_index = file_index * MAX_FILENAME_BYTES + byte_index;
            filenames[i][byte_index] = self.file_content_buffer[content_index];
        }
    }
    Ok((files_to_list, filenames))
    }
    ...
    pub fn inode_for(
        ...
        for file_index in 1..MAX_FILES_STORED {
            let mut match_found = true;
    
            for byte_index in 0..MAX_FILENAME_BYTES {
                let content_byte = self.file_content_buffer[file_index * MAX_FILENAME_BYTES + byte_index];
                let filename_byte = if byte_index < filename_len {
                    filename_bytes[byte_index]
                } else {
                    0 
                };
    
                if content_byte != filename_byte {
                    match_found = false;
                    break;
                }
    
                if content_byte == 0 && byte_index >= filename_len {
                    break;
                }
            }
    ...
    fn create_directory_entry(
        ...
        let entry_size = MAX_FILENAME_BYTES;
        let buf = &mut self.file_content_buffer;
    
        let mut offset = inode_num * MAX_FILENAME_BYTES;
        let mut found = false;
        let mut i = 0;
       ...
       /*  let base = offset + MAX_FILENAME_BYTES;
        ...
        if (offset + entry_size) >= directory_inode.bytes_stored as usize{
            if ( offset+entry_size) > (self.bytes_per_block()*directory_inode.blocks_used()) {
            ...
            directory_inode.bytes_stored = offset as u16 + entry_size as u16;
        }
        //println!("{:?}", self.file_content_buffer);
    ...
    pub fn close(&mut self, fd: usize) -> Result<(), FileSystemError> {
        ...
        let file_info = self.open[fd].unwrap();
    ...
    pub fn read(&mut self, fd: usize, buffer: &mut [u8]) -> Result<usize, FileSystemError> {
        ...
        let file_info = self.open.get_mut(fd).ok_or(FileSystemError::FileNotOpen)?;
        let file_info = file_info.as_mut().ok_or(FileSystemError::FileNotOpen)?;
        
        if file_info.writing {
            return Err(FileSystemError::NotOpenForRead);
        }
        let file_size = file_info.inode.bytes_stored as usize;
        if file_info.offset >= file_size {
            return Ok(0);
        }

        let available = file_size - file_info.offset;
        ...
        while bytes_read < to_read {
            ...
            let absolute_pos = file_info.offset;
            let block_index = absolute_pos / BLOCK_SIZE;
            let block_offset = absolute_pos % BLOCK_SIZE;
   
    
            let block_id = file_info.inode.blocks[block_index];
    
            self.disk.read(block_id as usize, &mut file_info.block_buffer);
    
  
            let bytes_left_in_block = BLOCK_SIZE - block_offset;
            let bytes_left_to_copy = to_read - bytes_read;
            let bytes_to_copy = bytes_left_in_block.min(bytes_left_to_copy);
    
 
            buffer[bytes_read..bytes_read + bytes_to_copy]
                .copy_from_slice(&file_info.block_buffer[block_offset..block_offset + bytes_to_copy]);
    ...
    pub fn write(&mut self, fd: usize, buffer: &[u8]) -> Result<(), FileSystemError> {
        ...
        let mut file_info = self.open[fd].unwrap();
    ...
    for i in buffer_offset..buffer.len() {
        ...
        if (i + file_info.offset) % BLOCK_SIZE == 0 && (i + file_info.offset) != 0{
            /*if file_info.inode.blocks_used() + 1 > MAX_FILE_BLOCKS{
                Err(FileSystemError::FileTooBig(self.max_file_size()));
            }*/
            self.disk.write(file_info.inode.blocks[file_info.current_block] as usize, &file_info.block_buffer);
            /*if file_info.current_block + 1 > MAX_FILE_BLOCKS{
                Err(FileSystemError::FileTooBig(MAX_FILE_BYTES));
            }*/
            let data_block = self.request_data_block().unwrap();
            file_info.current_block += 1;
            file_info.inode.blocks[file_info.current_block] = data_block;
            file_info.block_buffer = [0;BLOCK_SIZE];
            self.open[fd] = Some(file_info);
        
        }
        file_info.block_buffer[(i +file_info.offset)%BLOCK_SIZE] = buffer[i];
        }
        self.disk.write(file_info.inode.blocks[file_info.current_block] as usize, &file_info.block_buffer);
        file_info.offset += buffer.len(); 
        if file_info.offset > file_info.inode.bytes_stored as usize{
    ...
    pub fn open_create_reset(
        ...
        for i in 0..MAX_FILE_BLOCKS {
            ...
            self.disk.read(block_id as usize, &mut block_buffer);

            block_buffer.fill(0);

            self.disk.write(block_id as usize, &block_buffer);
        }
    ...
    let offset = bytes % BLOCK_SIZE;

    let mut file_info = FileInfo::write_inside(inode, inode_num, current_block, offset);

    
    if current_block < MAX_FILE_BLOCKS && current_block < file_info.inode.blocks.len() {
        let block_num = file_info.inode.blocks[current_block] as usize;
        self.disk.read(block_num, &mut file_info.block_buffer);
    }
...
impl<const MAX_BLOCKS: usize, const BLOCK_SIZE: usize> Inode<MAX_BLOCKS, BLOCK_SIZE> {
    ...
    pub fn from_bytes(buffer: &[u8]) -> Self {
        let low_order = buffer[0] as u16;
        let high_order = buffer[1]as u16;
        let bytes_stored = (high_order as u16) << 8 | (low_order as u16);
